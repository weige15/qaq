...
- Sampled LLaMA artifact preparation and sampled LLaMA one-step router training exist, but sampled 16-value artifacts are explicitly not full quantized inference artifacts.
...
- Build the first-milestone LLaMA mode matrix only after full artifacts and runtime metadata show `artifact_ref_mode: full_tensor_index` and `mixed_precision_forward_applied: true` for quantized modes.
